...
function buildPrompt(input: GenerateStudyContentInput) {
  return [
    "AI知識の学習用に、日本語で1問だけ作成してください。",
    "画像から読み取れる内容とテキスト情報の両方を使い、実務や日常利用で再利用できる知識を優先してください。",
    "問題は『用語の意味』『仕組み』『活用例』『注意点』『比較』『リスク』のいずれかを扱ってください。",
    "補足情報に書かれた内容は、問題文の主材料として具体的に織り込んでください。",
    "自由メモに書かれた内容は、解説の主材料として具体的に反映してください。自由メモが空なら、画像と補足情報だけで解説してください。",
    "模範解答は短すぎず、復習時に理解が深まる密度にしてください。",
    "解説では暗記だけでなく、使いどころや間違えやすい点が分かる内容にしてください。",
    "指定した JSON schema に厳密に従って返してください。",
    "",
    `学習テーマ: ${input.productName || "未入力"}`,
    `コンテキスト: ${input.brandName || "未入力"}`,
    `補足情報: ${input.note || "未入力"}`,
    `自由メモ: ${input.memo || "未入力"}`,
  ].join("\n");
}
...
export async function generateStudyContent(
  ...
  try {
    const response = await client.models.generateContent({
      ...
      config: {
        systemInstruction:
          "あなたはAI知識の学習コーチです。画像と補足情報から、復習効果の高い学習問題を1つ作成してください。",
