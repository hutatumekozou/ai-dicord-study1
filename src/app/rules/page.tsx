...
const ruleSections = [
  {
    title: "基本ルール",
    items: [
      "このアプリは、AIに関する知識問題を作って Discord で復習するための学習アプリです。",
  ...
  },
  {
    title: "問題登録ルール",
    items: [
      "学習テーマは後から見返して分かる名前で入れます。Discord の問題見出しにも表示されます。",
      "問題文にはAI用語、仕組み、活用例、注意点などを問う内容を書き、解答には答えをそのまま入れます。",
  ...
  },
  {
    ...
    items: [
      ...
      "解答メッセージのあとに「超正解」「大正解」「正解」「不正解」「手動」で結果を記録できます。",
      "カテゴリ名だけを送ると、そのカテゴリの中から苦手問題を最大 5 問まとめて出題します。",
      "コンテキストだけを送ると、そのコンテキストの中から苦手問題を最大 10 問まとめて出題します。",
      "「超正解」は 30 日後、「大正解」は 14 日後、「正解」は 7 日後、「不正解」は翌日に再出題されます。",
  ...
  },
  {
    ...
    items: [
      ...
      "自動送信は基本的に毎日18時に、その日の対象問題を上限件数まで順番に送ります。",
  ...
  },
  {
    ...
    items: [
      ...
      "動作が怪しいときは、まずローカルサイトまたは公開URLでアプリが開けるかを確認します。",
    ],
  },
  {
    title: "料金の確認方法",
    items: [
      "Google API の利用状況は Google AI Studio の「使用状況」で確認します。APIキーがこのアプリのものと一致しているかも合わせて見ます。",
      "Google API の料金設定や請求先は Google AI Studio の「利用額」「課金」から確認します。請求先未設定なら本格課金前です。",
      "Vercel の使用量は Vercel Dashboard の「Usage」で確認します。特に Blob Data Transfer、Blob Data Storage、Function Invocations を優先して見ます。",
      "画像が増えると Vercel Blob の保存量と転送量が増えやすいです。AI再生成や新規登録が増えると Google API 側の利用も増えます。",
    ],
  },
];
