...
`GEMINI_API_KEY` や Discord 関連の環境変数が未設定でも、管理画面の起動と閲覧自体は可能です。
...
| `DISCORD_BOT_TOKEN` | Discord返信/送信時に必須 | Bot トークン |
| `DISCORD_DEFAULT_USER_ID` | push送信時に推奨 | 単一ユーザー運用時の既定送信先 Discord userId |
| `DISCORD_STUDY_CHANNEL_ID` | サーバー運用時に推奨 | 学習専用チャンネルの channelId |
...
2. Bot token を発行し、`.env` の `DISCORD_BOT_TOKEN` に設定
3. 自分の Discord userId を `.env` の `DISCORD_DEFAULT_USER_ID` に設定
4. サーバー運用する場合は `.env` の `DISCORD_STUDY_CHANNEL_ID` に学習専用チャンネルIDを設定
5. 別プロセスで Bot を起動
...
`DISCORD_DEFAULT_USER_ID` は push 配信用です。未設定でも、DB 上の対象ユーザーに `discordUserId` が入っていれば送信できます。
単一ユーザーMVPとして運用する場合は `.env` に設定しておくのが安全です。
専用チャンネル運用では `解答87` `正解87` `不正解87` のように問題番号付きで返信します。
...
- DM受信: Discord Bot プロセスを別ホストまたは VPS で常駐
...
npm run send:due    # 本日送信対象をまとめてDiscordへ送信
npm run discord:bot # Discord DM受信Botを起動
```
...
### `Discord返信/送信の設定が不足しています`

- `.env` の `DISCORD_BOT_TOKEN` を設定してください
- push送信先がDBにない場合は `DISCORD_DEFAULT_USER_ID` も設定してください
...
- `.env` の `DISCORD_DEFAULT_USER_ID` を設定する
- もしくは対象ユーザーの `discordUserId` をDBへ登録する
...
- `APP_BASE_URL` の公開URLが失効していると、Discordは画像を取得できません
...
- AI 生成履歴の版管理は未実装
- 問題の一括再生成や一括編集は未実装
- Discord userId の取得を UI から行う導線は未実装
...
5. Discord 上で簡易メニューや学習状況サマリーを返す
